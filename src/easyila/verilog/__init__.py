...
def verilog_to_model(
    ...
    for mod_name in needed_submodules:
        submodules[mod_name] = _verilog_model_helper(
            ...
            all_signals,
            important_signals,
            2, # TODO figure out appropriate depth?
            coi_conf,
            submodules,
            inline_renames
        )
...
def _verilog_model_helper(
    ...
    names. `mod_depth` represents the number of scopes deep we're in, e.g. if we're generating
    ...
    for s in important_signals:
        ...
        if not inline_renames or not signaltype.isRename(termtype):
            ...
            # or different modules of the same depth
            if len(sc) < mod_depth + 1:
                continue
    ...
    return Model(
        ...
        instances={
            i_name: Instance(submodules[m_name], instance_inputs[i_name])
            for i_name, m_name in instance_names.items()
        },
        instructions={},
        init_values={
