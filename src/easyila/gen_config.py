...
def generate_portconfig(signals, target_dir, root=ROOT):
    ...
    for signal in signals:
        if signal.bounds:
            for index in range(signal.bounds[0], signal.bounds[1] + 1):
                acc += "\t\\\n\tCSV_DECL_STMT({}, {})".format(
                        signal.get_qualified_path() + f"[{index}]",
                        signal.width
                        )
        else:
            acc += "\t\\\n\tCSV_DECL_STMT({}, {})".format(signal.get_qualified_path(), signal.width)
    acc += "\n\n"
    acc += "#define CSV_ADD_DTRACE_STMTS"
    for signal in signals:
        if signal.bounds:
            for index in range(signal.bounds[0], signal.bounds[1] + 1):
                acc += "\t\\\n\tCSV_VAR_STMT({})".format(signal.get_qualified_path() + f"[{index}]")
        else:
            acc += "\t\\\n\tCSV_VAR_STMT({})".format(signal.get_qualified_path())
    acc += "\n"
